...
type RoutineStepDraft = { title: string; duration_minutes?: number };

function EditRoutineClient() {
  ...
  const [steps, setSteps]             = useState<RoutineStepDraft[]>([{ title: '' }]);
  ...
  useEffect(() => {
    ...
    routineAdapter.getRoutine(id).then(result => {
      if (result.ok) {
        const routine = result.data;
        setTitle(routine.title);
        setTimeOfDay(routine.time_of_day || 'morning');
        setSchedule(routine.schedule_type || 'daily');
        setSparkValue(routine.spark_value ?? 2);
        setDimensions((routine.value_dimensions as ValueDimensionId[]) ?? []);
        setChildId(routine.child_id ?? '');
        setSteps(routine.steps.length > 0 ? routine.steps.map(s => ({ title: s.title, duration_minutes: s.duration_minutes })) : [{ title: '' }]);
      } else {
  ...
  function toggleDimension(dimId: ValueDimensionId) {
    setDimensions(prev =>
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!id || !title.trim() || !family?.id || !profile?.id) return;
    setError('');
    setLoading(true);

    const validSteps = steps
      .filter(s => s.title.trim())
      .map((s, i) => ({ position: i + 1, title: s.title.trim(), duration_minutes: s.duration_minutes }));

    const result = await routineAdapter.updateRoutine(
      id,
      {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-display text-2xl text-stone-800">Editar rutina</h1>
      </div>
